...
$active_tab = $_GET['tab'] ?? 'stk';
echo '<div class="wrap"><h1>Super Talented Kids</h1>';

if ($active_tab === 'stk') {
    $stk_list_table = new STK_List_Table();
    $stk_list_table->prepare_items();

    // Filters + Search UI
?>
    <form method="get" id="wgt-filter-form" class="wgt-modern-filters">
        <input type="hidden" name="page" value="wgt" />
        <input type="hidden" name="tab" value="stk" />
