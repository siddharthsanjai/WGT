...
$active_tab = $_GET['tab'] ?? 'appreciation';
echo '<div class="wrap"><h1>Appreciation Awards</h1>';

if ($active_tab === 'appreciation') {
    $appreciation_list_table = new Appreciation_List_Table();
    $appreciation_list_table->prepare_items();

    // Filters + Search UI
?>
    <form method="get" id="wgt-filter-form" class="wgt-modern-filters">
        <input type="hidden" name="page" value="wgt" />
        <input type="hidden" name="tab" value="appreciation" />
        ...
        <div class="wgt-filters-grid">
            ...
            <div class="wgt-filter">
                ...
                <select name="category">
                    ...
                    <option value="2"<?php selected($_GET['category'] ?? '', '2'); ?>>Duo</option>
                    <option value="3"<?php selected($_GET['category'] ?? '', '3'); ?>>Group</option>
                    <option value="4"<?php selected($_GET['category'] ?? '', '4'); ?>>Mass</option>
                    <option value="5"<?php selected($_GET['category'] ?? '', '5'); ?>>Commercial</option>
                </select>
            </div>
...
<?php
    ...
    $appreciation_list_table->display();
    echo '</div>';
}
echo '</div>';
?>