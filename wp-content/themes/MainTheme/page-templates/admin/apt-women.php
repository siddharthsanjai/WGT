...
$active_tab = $_GET['tab'] ?? 'apt-women';
echo '<div class="wrap"><h1>APT Women</h1>';

if ($active_tab === 'apt-women') {
    $aptw_list_table = new Aptw_List_Table();
    $aptw_list_table->prepare_items();

    // Filters + Search UI
?>
    <form method="get" id="wgt-filter-form" class="wgt-modern-filters">
        <input type="hidden" name="page" value="wgt" />
        <input type="hidden" name="tab" value="apt-women" />
