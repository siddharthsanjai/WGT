...
class IhRecordManager
{
    ...
    public function __construct()
    {
        ...
        global $wpdb;
        $this->record_table = $wpdb->prefix . 'inspiring_humans';
        ...
        add_action('init', array($this, 'handle_ih_submission'));
        add_action('init', array($this, 'register_ih_endpoint'));
        add_filter('woocommerce_account_menu_items', array($this, 'add_ih_my_account_page'), 10, 2);
        add_action('woocommerce_account_inspiring-human_endpoint', array($this, 'ih_my_account_page'));
        add_action('wp_ajax_edit_inspiring_section', array($this, 'edit_inspiring_section'));
    }
    ...
    public function create($data)
    {
        ...
        $arg = [
            ...
            'category_fee_id'    => wgt_get_cat_fee_id($data['applying_as'], wgt_get_country_name_by_code($data['country']), 'inspiring-humans'),
        ...
        if ($insert) {
            ...
            wgt_send_application_received_email_to_user($arg, 'inspiring_humans');
        }
    ...
    protected function generateApplicationId()
    {
        ...
        $latest = $wpdb->get_var(
            ...
            WHERE application_id LIKE 'IBRIH%' 
            ORDER BY created_at DESC 
            LIMIT 1"
        );

        if (preg_match('/IBRIH(\d+)/', $latest, $matches)) {
    ...
    function handle_ih_submission()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['wgt_ih_submit'])) {
            if (!isset($_POST['wgt_ih_nonce']) || !wp_verify_nonce($_POST['wgt_ih_nonce'], 'wgt_ih_nonce_submit')) {
                wp_die('Security check failed');
            }

            $data = $_POST;
            global $ih_record_manager;
            if (!empty($data['record_id'])) {
                // UPDATE
                $updated = $ih_record_manager->update($data['record_id'], $data);
                if ($updated) {
                ...
                // CREATE
                $application_id = $ih_record_manager->create($data);
                if ($application_id) {
    ...
    function register_ih_endpoint()
    {
        add_rewrite_endpoint('inspiring-human', EP_ROOT | EP_PAGES);
    }

    function add_ih_my_account_page($items)
    {
        ...
        foreach ($items as $key => $label) {
            ...
            if ($key === 'super-talented-kid') {
                $new['inspiring-human'] = 'Inspiring Humans';
            }
        }
        return $new;
    }

    function ih_my_account_page()
    {
        include get_stylesheet_directory() . '/woocommerce/myaccount/inspiring-humans.php';
    }

    function update_holder_data($record_id, $data, $module = 'inspiring-human')
    {
        global $wpdb;
    ...
    function edit_inspiring_section()
    {
        global $wpdb, $common_class;
        ...
        switch ($section) {
            case 'basicdetails':
                $fields = [
                    ...
                    'category_fee_id'       => wgt_get_cat_fee_id($_POST['applying_as'], $get_records['country'], 'inspiring-humans'),
                ...
                $update1 = $this->update_holder_data($record_id, $fields1, 'inspiring-human');
                break;
            case 'addressdetails':
                $fields = [
                    ...
                    'cc'      => sanitize_text_field($_POST['phone_number_code'] ?? ''),
            ...
            case 'recorddetails':
                $slug   = $common_class->get_unique_slug($record_id, $this->record_table, sanitize_text_field(! empty($_POST['slug']) ? $_POST['slug'] : 'inspiring-humans'));
                $fields = [
        ...
        if (isset($data['featured'])) {
            ...
            if (!empty($data['featured']['src'])) {
                ...
                $return = $wpdb->insert(
                    ...
                    [
                        'id'             => wp_generate_uuid4(),
                        'imageable_type' => 'inspiring-human',
                        'imageable_id'   => $record_id,
                        'type'           => 'Featured',
        ...
        if (!empty($data) && is_array($data)) {
            ...
            foreach ($data as $index => $vid) {
                ...
                $return = $wpdb->insert(
                    ...
                    [
                        'id'                => wp_generate_uuid4(),
                        'videoable_type'    => 'inspiring-human',
                        'videoable_id'      => $record_id,
                        'src'               => $vid,
                        'created_at'        => $now,
                        'updated_at'        => $now,
...
$GLOBALS['ih_record_manager'] = new IhRecordManager();
