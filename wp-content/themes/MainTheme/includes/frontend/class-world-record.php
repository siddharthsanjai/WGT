...
class WorldRecordManager
{
    ...
    public function __construct()
    {
        ...
        global $wpdb;
        $this->record_table = $wpdb->prefix . 'records';
        ...
        add_action('init', array($this, 'handle_world_record_submission'));
        add_action('init', array($this, 'register_wr_endpoint'));
        add_filter('woocommerce_account_menu_items', array($this, 'add_wr_my_account_page'), 10, 2);
        add_action('woocommerce_account_record_endpoint', array($this, 'wr_my_account_page'));
        add_action('wp_ajax_edit_record_section', array($this, 'save_record_section'));
    }
    ...
    public function create($data)
    {
        ...
        $arg = [
            ...
            'category_fee_id'    => wgt_get_cat_fee_id($data['applying_as'], wgt_get_country_name_by_code($data['country']), 'records'),
            'purpose_id'         => intval($data['purpose']),
            'title'              => sanitize_text_field($data['title']),
            'attempt_title'      => '',
            'slug'               => NULL,
            'date'               => date('Y-m-d', strtotime($data['attempt_date'] ?? '')),
            'estimate_participants' => sanitize_text_field($data['estimate_participants'] ?? NULL),
        ...
        if ($insert) {
            ...
            wgt_send_application_received_email_to_user($arg, 'record');
        }
    ...
    public function update($record_id, $data)
    {
        ...
        $update = $wpdb->update(
            ...
            [
                'title' => sanitize_text_field($data['title']),
                'attempt_title' => sanitize_text_field($data['title']),
                'state' => sanitize_text_field($data['state']),
                'city' => sanitize_text_field($data['city']),
                'address' => sanitize_text_field($data['address']),
                'zipcode' => sanitize_text_field($data['zip']),
    ...
    protected function generateApplicationId()
    {
        ...
        $latest = $wpdb->get_var(
            ...
            WHERE application_id LIKE 'IBR%' 
            ORDER BY created_at DESC 
            LIMIT 1"
        );

        if (preg_match('/IBR(\d+)/', $latest, $matches)) {
    ...
    function handle_world_record_submission()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['wgt_application_submit'])) {
            if (!isset($_POST['wgt_record_nonce']) || !wp_verify_nonce($_POST['wgt_record_nonce'], 'wgt_record_nonce_submit')) {
                wp_die('Security check failed');
            }

            $data = $_POST;
            global $world_record_manager;
            if (!empty($data['record_id'])) {
                // UPDATE
                $updated = $world_record_manager->update($data['record_id'], $data);
                if ($updated) {
                ...
                // CREATE
                $application_id = $world_record_manager->create($data);
                if ($application_id) {
    ...
    function register_wr_endpoint()
    {
        add_rewrite_endpoint('record', EP_ROOT | EP_PAGES);
    }

    function add_wr_my_account_page($items)
    {
        ...
        foreach ($items as $key => $label) {
            ...
            if ($key === 'dashboard') {
                $new['record'] = 'World Records';
            }
        }
        return $new;
    }

    function wr_my_account_page()
    {   
        include get_stylesheet_directory() . '/woocommerce/myaccount/world-records.php';
    }

    function update_holder_data($record_id, $data, $module = 'records')
    {
        global $wpdb;
    ...
    function get_record_with_holders($record_id)
    {
        ...
        $payment = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM {$this->payment_table} WHERE payable_id = %s",
                $record_id
            ),
            ARRAY_A
        );
        ...
        switch ($section) {
            case 'basicdetails':
                $fields = [
                    ...
                    'category_fee_id'       => wgt_get_cat_fee_id($_POST['applying_as'], $get_records['country'], 'records'),
                ...
                $update1 = $this->update_holder_data($record_id, $fields1, 'records');
                break;
            case 'addressdetails':
                $fields = [
            ...
            case 'recorddetails':
                $slug   = $common_class->get_unique_slug($record_id, $this->record_table, sanitize_text_field(! empty($_POST['slug']) ? $_POST['slug'] : 'records'));
                $fields = [
    ...
    function get_all_applications_by_user($user_id)
    {
        global $wpdb;
        $records_table = $this->record_table;   // main records table
        $payment_table = $this->payment_table; 
        ...
        $records = $wpdb->get_results(
            $wpdb->prepare("SELECT rt.*,pt.status as payment_status FROM {$records_table} as rt LEFT JOIN {$payment_table} as pt ON pt.payable_id=rt.id WHERE user_id = %d ORDER BY created_at DESC", $user_id),
            ARRAY_A
        );
...
$GLOBALS['world_record_manager'] = new WorldRecordManager();
