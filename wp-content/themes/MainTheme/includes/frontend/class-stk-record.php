...
class StkRecordManager
{
    ...
    public function __construct()
    {
        ...
        global $wpdb;
        $this->record_table = $wpdb->prefix . 'super_talented_kids';
        ...
        add_action('init', array($this, 'handle_stk_submission'));
        add_action('init', array($this, 'register_stk_endpoint'));
        add_filter('woocommerce_account_menu_items', array($this, 'add_stk_my_account_page'), 10, 2);
        add_action('woocommerce_account_super-talented-kid_endpoint', array($this, 'stk_my_account_page'));
        add_action('wp_ajax_edit_stk_section', array($this, 'save_record_section'));
    }
    ...
    public function create($data)
    {
        ...
        $arg = [
            ...
            'category_fee_id'    => wgt_get_cat_fee_id($data['applying_as'], wgt_get_country_name_by_code($data['country']), 'super-talented-kids'),
        ...
        if ($insert) {
            ...
            wgt_send_application_received_email_to_user($arg, 'super_talented_kids');
        }
    ...
    protected function generateApplicationId()
    {
        ...
        $latest = $wpdb->get_var(
            ...
            WHERE application_id LIKE 'IBRSTK%' 
            ORDER BY created_at DESC 
            LIMIT 1"
        );

        if (preg_match('/IBRSTK(\d+)/', $latest, $matches)) {
    ...
    function handle_stk_submission()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['wgt_stk_submit'])) {
            if (!isset($_POST['wgt_stk_nonce']) || !wp_verify_nonce($_POST['wgt_stk_nonce'], 'wgt_stk_nonce_submit')) {
                wp_die('Security check failed');
            }

            $data = $_POST;
            global $stk_record_manager;
            if (!empty($data['record_id'])) {
                // UPDATE
                $updated = $stk_record_manager->update($data['record_id'], $data);
                if ($updated) {
                ...
                // CREATE
                $application_id = $stk_record_manager->create($data);
                if ($application_id) {
    ...
    function register_stk_endpoint()
    {
        add_rewrite_endpoint('super-talented-kid', EP_ROOT | EP_PAGES);
    }

    function add_stk_my_account_page($items)
    {
        ...
        foreach ($items as $key => $label) {
            ...
            if ($key === 'record') {
                $new['super-talented-kid'] = 'Super Talented Kid';
            }
        }
        return $new;
    }

    function stk_my_account_page()
    {
        include get_stylesheet_directory() . '/woocommerce/myaccount/super-talented-kids.php';
    }

    function update_holder_data($record_id, $data, $module = 'super-talented-kids')
    {
        global $wpdb;
        ...
        switch ($section) {
            case 'basicdetails':
                $fields = [
                    ...
                    'category_fee_id'       => wgt_get_cat_fee_id($_POST['applying_as'], $get_records['country'], 'super-talented-kids'),
                ...
                $update1 = $this->update_holder_data($record_id, $fields1, 'super-talented-kids');
                break;
            case 'addressdetails':
                $fields = [
            ...
            case 'recorddetails':
                $slug   = $common_class->get_unique_slug($record_id, $this->record_table, sanitize_text_field(! empty($_POST['slug']) ? $_POST['slug'] : 'super-talented-kids'));
                $fields = [
        ...
        if (isset($data['featured'])) {
            ...
            if (!empty($data['featured']['src'])) {
                ...
                $return = $wpdb->insert(
                    ...
                    [
                        'id'             => wp_generate_uuid4(),
                        'imageable_type' => 'super-talented-kids',
                        'imageable_id'   => $record_id,
                        'type'           => 'Featured',
        ...
        if (!empty($data) && is_array($data)) {
            ...
            foreach ($data as $index => $vid) {
                ...
                $return = $wpdb->insert(
                    ...
                    [
                        'id'                => wp_generate_uuid4(),
                        'videoable_type'    => 'super-talented-kids',
                        'videoable_id'      => $record_id,
                        'src'               => $vid,
                        'created_at'        => $now,
                        'updated_at'        => $now,
    ...
    function create_payment($record_id, $data)
    {
        ...
        $insert = $wpdb->insert($this->payment_table, [
            ...
            'payable_type'         => 'super-talented-kid',
...
$GLOBALS['stk_record_manager'] = new StkRecordManager();
