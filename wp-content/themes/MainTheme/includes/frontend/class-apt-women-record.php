...
class AptwRecordManager
{
    ...
    public function __construct()
    {
        ...
        global $wpdb;
        $this->record_table  = $wpdb->prefix . 'apt_women';
        ...
        add_action('init', array($this, 'handle_aptw_submission'));
        add_action('init', array($this, 'register_aptw_endpoint'));
        add_filter('woocommerce_account_menu_items', array($this, 'add_aptw_my_account_page'), 10, 2);
        add_action('woocommerce_account_apt-womens_endpoint', array($this, 'aptw_my_account_page'));
        add_action('wp_ajax_edit_apt_women_section', array($this, 'edit_apt_women_section'));
    }
    ...
    public function create($data)
    {
        ...
        $arg = [
            ...
            'category_fee_id'    => wgt_get_cat_fee_id($data['applying_as'], wgt_get_country_name_by_code($data['country']), 'apt-women'),
        ...
        if ($insert) {
            ...
            wgt_send_application_received_email_to_user($arg, 'apt_women');
        }
    ...
    protected function generateApplicationId()
    {
        ...
        $latest = $wpdb->get_var(
            ...
            WHERE application_id LIKE 'IBRAW%' 
            ORDER BY created_at DESC 
            LIMIT 1"
        );

        if (preg_match('/IBRAW(\d+)/', $latest, $matches)) {
    ...
    function handle_aptw_submission()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['wgt_aptw_submit'])) {
            if (!isset($_POST['wgt_aptw_nonce']) || !wp_verify_nonce($_POST['wgt_aptw_nonce'], 'wgt_aptw_nonce_submit')) {
                wp_die('Security check failed');
            }

            $data = $_POST;
            global $aptw_record_manager;
            if (!empty($data['record_id'])) {
                // UPDATE
                $updated = $aptw_record_manager->update($data['record_id'], $data);
                if ($updated) {
                ...
                // CREATE
                $application_id = $aptw_record_manager->create($data);
                if ($application_id) {
    ...
    function register_aptw_endpoint()
    {
        add_rewrite_endpoint('apt-womens', EP_ROOT | EP_PAGES);
    }

    function add_aptw_my_account_page($items)
    {
        ...
        foreach ($items as $key => $label) {
            ...
            if ($key === 'inspiring-human') {
                $new['apt-womens'] = 'Apt Women';
            }
        }
        return $new;
    }

    function aptw_my_account_page()
    {
        include get_stylesheet_directory() . '/woocommerce/myaccount/apt-women.php';
    }

    function update_holder_data($record_id, $data, $module = 'apt-women')
    {
        global $wpdb;
    ...
    function edit_apt_women_section()
    {
        global $wpdb, $common_class;
        ...
        switch ($section) {
            case 'basicdetails':
                $fields = [
                    ...
                    'category_fee_id'       => wgt_get_cat_fee_id($_POST['applying_as'], $get_records['country'], 'apt-women'),
                ...
                $update1 = $this->update_holder_data($record_id, $fields1, 'apt-women');
                break;
            case 'addressdetails':
                $fields = [
            ...
            case 'recorddetails':
                $slug   = $common_class->get_unique_slug($record_id, $this->record_table, sanitize_text_field(! empty($_POST['slug']) ? $_POST['slug'] : 'apt-women'));
                $fields = [
...
$GLOBALS['aptw_record_manager'] = new AptwRecordManager();