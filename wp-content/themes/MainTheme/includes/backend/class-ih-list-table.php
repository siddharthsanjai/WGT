...
class Ih_List_Table extends WP_List_Table
{
    private $cutoff_date;
    ...
    public function __construct()
    {
        ...
        parent::__construct([
            'singular' => 'inspiring_humans',
            'plural'   => 'inspiring_humans',
            'ajax'     => false,
        ]);
    ...
    public function prepare_items()
    {
        ...
        $count_sql = "SELECT COUNT(DISTINCT r.id)
                  FROM {$wpdb->prefix}inspiring_humans r
                  $where_sql";
        ...
        // Fetch data with payment status
        $data_sql = "
        SELECT r.*,
               (SELECT p.status 
    ...
    public function column_default($item, $column_name)
    {
        ...
        switch ($column_name) {
            ...
            case 'status':
                ...
                return '<select class="update_record" data-id="' . esc_attr($item['id']) . '" data-module="inspiring_humans" data-action_type="status">
                    <option ' . ($item['status'] == 'Active' ? 'selected' : '') . '>Active</option>
                    <option ' . ($item['status'] == 'Inactive' ? 'selected' : '') . '>Inactive</option>
                </select>';

            case 'lead_stage':
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="inspiring_humans" data-action_type="lead_stage" ' . ($item['status'] === 'Inactive' ? 'disabled' : '') . '>';
            ...
            case 'approval_status':
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="inspiring_humans" data-action_type="approval_status" ' . ($item['status'] === 'Inactive' ? 'disabled' : '') . '>';
            ...
            case 'email_triggers':
                ...
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="inspiring_humans" data-action_type="email_triggers" ' . (($conditions) ? 'disabled' : '') . '>';
            ...
            case 'remark':
                return '<input type="text" class="update_record" data-id="' . esc_attr($item['id']) . '" value="' . esc_attr($item['remark']) . '" data-module="inspiring_humans" data-action_type="remark"/>';

            case 'lead_owner':
                return '<input type="text" class="update_record" data-id="' . esc_attr($item['id']) . '" value="' . esc_attr($item['lead_owner']) . '" data-module="inspiring_humans" data-action_type="lead_owner"/>';
            ...
            case 'action':
                $record_id = $item['id'];
                return '
                <a class="button button-primary" href="' . 'admin.php?page=wgt&tab=inspiring-humans&section=basicdetails&id=' . esc_attr($record_id) . '">Edit</a><br>
            ';
