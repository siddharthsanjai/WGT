...
class STK_List_Table extends WP_List_Table
{
    private $cutoff_date;
    ...
    public function prepare_items()
    {
        ...
        // ✅ Search filter across both user tables
        if ($search) {
        ...
        $count_sql = "SELECT COUNT(DISTINCT r.id)
                  FROM {$wpdb->prefix}super_talented_kids r
                  $where_sql";
    ...
    public function column_default($item, $column_name)
    {
        ...
        switch ($column_name) {
            ...
            case 'status':
                ...
                return '<select class="update_record" data-id="' . esc_attr($item['id']) . '" data-module="super_talented_kids" data-action_type="status">
                    <option ' . ($item['status'] == 'Active' ? 'selected' : '') . '>Active</option>
                    <option ' . ($item['status'] == 'Inactive' ? 'selected' : '') . '>Inactive</option>
                </select>';

            case 'lead_stage':
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="super_talented_kids" data-action_type="lead_stage" ' . ($item['status'] === 'Inactive' ? 'disabled' : '') . '>';
            ...
            case 'approval_status':
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="super_talented_kids" data-action_type="approval_status" ' . ($item['status'] === 'Inactive' ? 'disabled' : '') . '>';
            ...
            case 'email_triggers':
                ...
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="super_talented_kids" data-action_type="email_triggers" ' . (($conditions) ? 'disabled' : '') . '>';
            ...
            case 'remark':
                return '<input type="text" class="update_record" data-id="' . esc_attr($item['id']) . '" value="' . esc_attr($item['remark']) . '" data-module="super_talented_kids" data-action_type="remark"/>';
            ...
            case 'action':
                $record_id = $item['id'];
                return '
                <a class="button button-primary" href="' . 'admin.php?page=wgt&tab=stk&section=basicdetails&id=' . esc_attr($record_id) . '">Edit</a<br>
            ';
