...
class Aptw_List_Table extends WP_List_Table
{
    private $cutoff_date;
    ...
    public function prepare_items()
    {
        ...
        $payment      = isset($_GET['payment_status']) ? sanitize_text_field($_GET['payment_status']) : '';
    ...
    public function column_default($item, $column_name)
    {
        ...
        switch ($column_name) {
            ...
            case 'status':
                ...
                return '<select class="update_record" data-id="' . esc_attr($item['id']) . '" data-module="apt-women" data-action_type="status">
                    <option ' . ($item['status'] == 'Active' ? 'selected' : '') . '>Active</option>
                    <option ' . ($item['status'] == 'Inactive' ? 'selected' : '') . '>Inactive</option>
                </select>';

            case 'lead_stage':
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="apt-women" data-action_type="lead_stage" ' . ($item['status'] === 'Inactive' ? 'disabled' : '') . '>';
            ...
            case 'approval_status':
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="apt-women" data-action_type="approval_status" ' . ($item['status'] === 'Inactive' ? 'disabled' : '') . '>';
            ...
            case 'email_triggers':
                ...
                $html = '<select class="update_record active_disable" data-id="' . esc_attr($item['id']) . '" data-module="apt_women" data-action_type="email_triggers" ' . (($conditions) ? 'disabled' : '') . '>';
            ...
            case 'remark':
                return '<input type="text" class="update_record" data-id="' . esc_attr($item['id']) . '" value="' . esc_attr($item['remark']) . '" data-module="apt-women" data-action_type="remark"/>';

            case 'lead_owner':
                return '<input type="text" class="update_record" data-id="' . esc_attr($item['id']) . '" value="' . esc_attr($item['lead_owner']) . '" data-module="apt-women" data-action_type="lead_owner"/>';
            ...
            case 'action':
                $record_id = $item['id'];
                return '
                <a class="button button-primary" href="' . 'admin.php?page=wgt&tab=apt-women&section=basicdetails&id=' . esc_attr($record_id) . '">Edit</a><br>
            ';
